...
turtles-own [
  ; --- Economic Variables ---
  ; [REMOVED] rate_of_fishing       ;; max capacity to fish per attempt    ;; [now fish everything from a patch; or randomized 70-100% e.g.]
...
globals [
  ...
  ; expedition_cost         ;; cost per fishing trip (deduct upon departing)             ;; (default at 3.00)
...
to go
  ...
  pollution-spread
  ;pollution-decay  ;; we removed pollution decay since we assume no decay, but instead the nurdles stuck on the beach
  fish-reproduce
  fisherman-behavior
...
;;; [OUTDATED] PREVIOUS STATIC POLLUION SPREAD
...
;;; [UNUSED]
; to pollution-decay
;  ask patches with [pollution_amount > 0] [
;
;    ;; Standard Decay
