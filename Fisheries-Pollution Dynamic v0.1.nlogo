...
patches-own [
  catch                 ;; latest amount of the fish caught on this patch
...
to pollution-spread
  ...
  ;; imagine a single patch of 100   polution in the middle
  ;;                            20   pollution remains in the middle.
  ;;                            20*4 pollution spread to 4 patch on Nth-Est-Wst-Sth

  ask patches with [pollution_amount > 0 and is_land = 0] [
    let share pollution_amount / 5
    ask neighbors4 with [is_land = 0] [
      set pollution_amount pollution_amount + share
      if pollution_amount > 100 [ set pollution_amount 100 ]
;    ask self [
;      set pollution_amount share
;      ]
    ]
  ]
end
...
to pollute
  ask n-of 200 patches with [is_land = 0] [                       ;; pollute 200 patches (~1.22%) out of 128*128 = 16,384 patches
    set pollution_amount pollution_amount + 100
    if pollution_amount > 100 [ set pollution_amount 100 ]
    recolor
  ]
end
